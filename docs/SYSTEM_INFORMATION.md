...
src/defcompile.rs    Shared per-process SynthDef output directory (osc.rs writes compiled bytes here for /d_load).
src/dsp/             Standalone SynthDef binary encoder — no sclang/scsynth. graph.rs (IR) + encoder.rs (scsyndef bytes) adapted from vibelang-dsp (MIT); ctx.rs is audion glue.
...
**Purpose**: Avoid re-walking/re-encoding unchanged SynthDefs on file reload during `--watch` mode. `synthdef::build_synthdef()` builds the UGen graph and encodes it to `scsyndef` bytes directly (`src/dsp/`, no sclang/scsynth subprocess involved), but it's still wasted work to redo on every reload if the `define` body hasn't changed.
...
- Cache hit → skip `build_synthdef()`, reuse bytes, print `"reusing cached synth 'name'"`
- Cache miss → call `build_synthdef()`, store bytes in cache, print `"defined synth 'name'"`
- **Always** loads sample buffers (even on cache hit) because `free_all_buffers()` clears them on reload
- Cached bytes are written to a file and loaded via `/d_load` (see `osc.rs::load_synthdef`)

**Buffer ID determinism**:
- Compiled SynthDef bytecode has buffer IDs hardcoded (baked in by `build_synthdef()` as literal Control defaults)
